defmodule ShortLetsWeb.PhotoControllerTest do
  use ShortLetsWeb.ConnCase

  alias ShortLets.Accounts

  @create_attrs %{apartment_id: "some apartment_id", image: "some image"}
  ...
  def fixture(:photo) do
    {:ok, photo} = Accounts.create_photo(@create_attrs)
    photo
  end
  ...
  describe "update photo" do
    ...
    test "redirects when data is valid", %{conn: conn, photo: photo} do
      ...
      assert html_response(conn, 200) =~ "some updated apartment_id"
    end
